/**
 * Weight Tracking Routes (Updated for Custom Schema)
 * 
 ...
 * - Uses weight_kg field instead of weight + unit
 * - Uses logged_at instead of measured_at
 * - Core functionality preserved with schema adaptations
 * 
...
/**
 * Weight log validation helper (Updated for simplified schema)
...
const validateWeightLog = (weightData) => {
    const errors = [];
    const { weightKg, loggedAt, notes } = weightData;
 ...
 * - sortBy: string (optional) - Sort by field (logged_at, weight_kg)
...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        // Apply sorting
        const validSortFields = ['logged_at', 'weight_kg'];
 ...
 * - bodyFatPercentage: number (optional) - Body fat percentage if available (if schema supports)
 * - muscleMass: number (optional) - Muscle mass if available (if schema supports)
 * 
 * Response:
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        // Validate optional body composition values (if schema supports them)
        ...
        // Prepare weight log data for your schema
        const weightLogData = {
            user_id: userId,
        ...
        // Add optional fields if your schema supports them
        if (notes) weightLogData.notes = notes.trim();
        if (bodyFatPercentage !== undefined) weightLogData.body_fat_percentage = parseFloat(bodyFatPercentage);
        if (muscleMass !== undefined) weightLogData.muscle_mass = parseFloat(muscleMass);
        
        // Add created_at if your schema supports it
        try {
            weightLogData.created_at = new Date().toISOString();
        } catch (e) {
            // Ignore if schema doesn't support created_at
        }
 ...
 * - bodyFatPercentage: number - Updated body fat percentage (if schema supports)
 * - muscleMass: number - Updated muscle mass (if schema supports)
 * 
 * Response:
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        // Add updated_at if schema supports it
        try {
            updateData.updated_at = new Date().toISOString();
        } catch (e) {
            // Ignore if schema doesn't support updated_at
        }
