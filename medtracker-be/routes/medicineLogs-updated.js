/**
 * Medicine Log Routes (Updated for Custom Schema)
 * 
 * Handles logging and tracking of medicine intake including:
 * - Recording when medicines are taken with timestamps (simplified schema)
 ...
 * - Simple medicine_logs table with id, medicine_id, taken_at
 * - Includes user_id for RLS (to be added to existing schema)
 * 
...
/**
 * Medicine log validation helper (Updated for simplified schema)
 ...
 * - dosageTaken: string (optional) - Actual dosage taken (for additional notes)
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        // Check for duplicate logs (same medicine within 5 minutes) - optional safety check
        ...
        // Prepare medicine log data for your simplified schema
        const logData = {
            user_id: userId,
        ...
        if (dosageTaken) logData.dosage_taken = dosageTaken.trim();
        if (notes) logData.notes = notes.trim();
        if (supabase.from('medicine_logs').select().limit(1).then(() => true).catch(() => false)) {
 ...
 * 
 * Update an existing medicine log (mainly for correcting the taken_at time)
 * Ensures user can only update their own medicine logs
 * 
 * Request body: (all fields optional)
 * - takenAt: string - Updated timestamp when medicine was taken
 * - dosageTaken: string - Updated dosage taken (if supported)
 * - notes: string - Updated notes (if supported)
 * 
 * Response:
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        // Only include these fields if your schema supports them
        ...
        // Add updated_at if your schema supports it
