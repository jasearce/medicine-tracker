/**
 * Medicine Management Routes (Updated for Custom Schema)
 * 
 * Handles all medicine-related CRUD operations including:
 * - Creating new medicines with interval and meal-based scheduling
 ...
 * - Managing interval-based schedules (every X hours/days)
 * - Managing meal-based schedules (before/after/with meals)
 * 
 * Schema compatibility:
 * - schedule_type: 'interval' or 'meal_based' 
 * - interval_minutes, interval_days for time-based scheduling
 * - meal_timing array for meal-based scheduling
 * 
...
/**
 * Medicine validation helper (Updated for new schema)
 * Validates medicine data according to the custom schema business rules
 * 
...
const validateMedicine = (medicineData) => {
    ...
    const { 
        name, 
        dose, // Using 'dose' to match your schema
        scheduleType, 
        intervalMinutes, 
        intervalDays, 
        mealTiming, 
    ...
    if (scheduleType === 'meal_based') {
        if (!mealTiming || !Array.isArray(mealTiming) || mealTiming.length === 0) {
            ...
            // Check for reasonable number of meal timings
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        // Prepare medicine data according to your schema
        const medicineData = {
            user_id: userId,
            name: name.trim(),
            dose: dose.trim(),
